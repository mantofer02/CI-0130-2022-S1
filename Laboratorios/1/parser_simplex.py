...
def parse_problem(objective, restrictions, maximize):
    ...
    for key in parsed_eq:
      values_array.append(parse_equation(objective).get(key))
    ...
    for i in range(len(restrictions)):
        if parsed_restrictions[i][2] == False:
          ...
          sign_m = -sign_m
    values_array.append(sign_m)

    #variable = dict parsed restriction


    for i in range(len(restrictions)):
      #recorro dict
      rvalues_array.append(parsed_restrictions[i][0].get("x" + str()))
